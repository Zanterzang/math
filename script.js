...
function calculate() {
    ...
    for (var c in colcs) {
        if (c==0) {
            ...
            if (Number(calcs[calcs.length-1])) {
                if (Number(colcs[c])||Number(colcs[c])===0) {
            ...
            } else if (calcs[calcs.length-1][0] !== ")") {
                if (Number(colcs[c])) {
    ...
    for (var b in brackets) {
        if (brackets[b][0]===")") {
            ...
            var o = 1;
            for (o=1; o<=b; o++) {
                if (brackets[1*b-o][0]==="(" && calcs[brackets[1*b-o][1]].length == 1) {
